...
#define TAG "com.miga.richedit"

using namespace v8;

static Persistent<Object> bindingCache;

static void TiRichedit_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TiRichedit.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::TiRicheditBindings::lookupGeneratedInit(
...
static void TiRichedit_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TiRichedit_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TiRichedit_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiRicheditBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiRicheditBinding = {
	"com.miga.richedit",
	TiRichedit_init,
	TiRichedit_dispose
};
...
Java_com_miga_richedit_TiRicheditBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.miga.richedit", &TiRicheditBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiRicheditBindings::lookupGeneratedInit));
}
